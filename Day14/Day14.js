...
const Reaction_1 = require("./../Classes/Reaction");
...
const reactions = Input_1.day14Input.map(r => new Reaction_1.Reaction(r));
const matList = {};
...
function addReactionMap(reaction) {
...
let mats = {};
//  * producir
function producir(chemical, cantidad) {
    if (!mats[chemical]) {
        mats[chemical] = 0;
    }
    if (mats[chemical] >= cantidad) {
        ...
        const produccionesNecesarias = Math.ceil((cantidad - mats[chemical]) / matList[chemical].out);
